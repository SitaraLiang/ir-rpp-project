...
def get_prefs(
    sample: int, runs: dict[str, util.trec_io.Run], measures: list[str], per_query: bool
    ...
    try:
        preference_file = open("preferences.jsonl", "w")
        metric_file = open("metrics.jsonl", "w")
            ...
            for i in range(len(runids)):
                ...
                for j in range(i + 1, len(runids)):
                    ...
                    for m in measures:
                        ...
                        if pref is None:
                            ...
                            for k in range(len(rv_i.positions)):
                                ...
                                sys.stderr.write(
                                    f"rv_i[{k}].did={rv_i.positions[k].did}\n"
                                )
                                sys.stderr.write(
                            ...
                            for k in range(len(rv_j.positions)):
                                ...
                                sys.stderr.write(
                                    f"rv_j[{k}].did={rv_j.positions[k].did}\n"
                                )
                                sys.stderr.write(
                    ...
                    if per_query:
                        ...
                        preference_file.write(json.dumps(output_object) + "\n")
                ...
                if per_query:
                    ...
                    if output_object is not None:
                        # print(json.dumps(output_object))
                        metric_file.write(json.dumps(output_object) + "\n")
    finally:
...
if __name__ == "__main__":
    parser: argparse.ArgumentParser = argparse.ArgumentParser(
        sys.argv[0], formatter_class=RawTextHelpFormatter
    )
    ...
    parser.add_argument(
        ...
        "-m",
        dest="measures",
        help="preference-based evaluation: rpp, invrpp, dcgrpp, lexirecall, lexiprecision, rrlexiprecision\nmetric-based evaluation: ap, rbp, rr, ndcg, rp, p@k, r@k",
        action="append",
    )
    parser.add_argument(
        "--measure_set",
        "-M",
        dest="measure_set",
        help="preferences, all, none",
        default="none",
    )
    parser.add_argument(
        "--binary_relevance",
        "-b",
        dest="binary_relevance",
        help="binary relevance based on supplied grade (i.e., item is considered relevant if its grade is >= the supplied grade; non-relevant otherwise).",
    )
    parser.add_argument(
        "--relevance_floor",
        "-f",
        dest="relevance_floor",
        help="any below this value is 0",
    )
    parser.add_argument(
        "--query_eval_wanted",
        "-q",
    ...
    parser.add_argument(
        "--label_fraction",
        dest="label_fraction",
        help="fraction of labels to preserve (default = 1.0)",
        type=float,
        default=1.0,
    )
    parser.add_argument(
        "--label_fraction_policy",
        dest="label_fraction_policy",
        help="how to sample labels (random, pool; default=random)",
        type=str,
        default="random",
    )
    parser.add_argument(
        "--samples", dest="samples", help="number of samples", type=int, default=1
    )
    ...
    binary_relevance = (
        None if args.binary_relevance is None else float(args.binary_relevance)
    )
    relevance_floor = (
        None if args.relevance_floor is None else float(args.relevance_floor)
    )
    topk = None if args.topk is None else args.topk

    for sample in range(args.samples):
        qrels: util.trec_io.QRels = util.trec_io.read_qrels(
            args.qrels, binary_relevance, relevance_floor
        )
        ...
        if args.label_fraction < 1.0:
            if args.label_fraction_policy == "pool":
                qrels_pool_frequencies = util.trec_io.compute_qrel_pool_frequencies(
                    args.runfiles, qrels
                )
